...
<div class="card" id="employeeTable">
    <div class="card-body">
        <div class="flex items-center gap-3 mb-4">
            <h6 class="text-15 grow">Kelompok Tani(<b class="total-Employs">{{ $totalData }}</b>)</h6>
            <div class="shrink-0">
            ...
            <tr>
                ...
                <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Action">
                    <div class="flex gap-3">
                        <a href="{{ route('kelompokTani.edit', $kt->id_kelompok_tani) }}" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 edit-item-btn bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"><i data-lucide="pencil" class="size-4"></i></a>
                        <form id="deleteForm_{{ $kt->id_kelompok_tani }}" action="{{ route('kelompokTani.delete', $kt -> id_kelompok_tani)}}" method="POST">
                            @csrf
                            <button type="submit" class="toggle-status flex items-center justify-center text-white transition-all duration-200 ease-linear rounded-md size-8 hover:text-white 
                                                    bg-red-500 delete-btn" data-id="{{ $kt->id_kelompok_tani }}">
            ...
            <form class="create-form" id="create-form" action="/tambah-kelompok-tani" method="POST">
                ...
                <div class="xl:col-span-12">
                    ...
                    <select required class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" name="kecamatan" id="kecamatan">
                        ...
                        <option value="{{ $k->dis_id }}">{{ $k->dis_name }}</option>
                        @endforeach
                    </select>
                </div>
