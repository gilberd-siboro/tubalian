...
<div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
    <ul class="flex flex-wrap items-center gap-2 mb-3 font-normal text-15">
        <li class="relative before:content-['\ea54'] before:font-remix before:ltr:-right-1 before:rtl:-left-1 before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:rtl:rotate-180 before:text-slate-500 dark:before:text-zink-200">
            <a href="/admin-bantuan" class="text-slate-500 dark:text-zink-200">Bantuan</a>
        </li>
...
<div class="card mx-auto lg:w-2/3">
    <div class="card-body">
        <form action="{{ route('Adminbantuan.update', $bantuan -> id_bantuan)}}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                <div class="mb-3 md:col-span-2">
                    <label for="jenis" class="inline-block mb-2 text-base font-medium">Jenis Bantuan</label>
                    <input required type="text" id="jenis" name="jenis" value="{{ old('jenis', $bantuan->jenis_bantuan) }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                </div>
                <div class="mb-3">
                    <label class="inline-block mb-2 text-base font-medium">Tanggal </label>
                    <input required type="date" class="date-input form-input border-slate-200 dark:border-zink-500" value="{{ \Carbon\Carbon::parse($bantuan->tanggal)->format('Y-m-d') }}">
                    <input required type="hidden" name="tanggal" class="formatted-date-input">
                </div>
                <div class="mb-3">
                    <p class="formatted-date-display">Tanggal: {{ $bantuan->tanggal }}</p>
                </div>
                <div class="mb-3 md:col-span-2">
                    ...
                    <select required class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" name="kel_tani" id="kel_tani">
                        ...
                        <option value="{{ $kt->id_kelompok_tani }}" {{ $kt->id_kelompok_tani == $bantuan->id_kelompok_tani ? 'selected' : '' }}>{{ $kt->nama_kelompok_tani }} </option>
                        @endforeach
                    </select>
                </div>
