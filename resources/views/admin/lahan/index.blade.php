...
<div class="card" id="employeeTable">
    <div class="card-body">
        <div class="flex items-center gap-3 mb-4">
            <h6 class="text-15 grow"> Lahan(<b class="total-Employs">{{ $totalData }}</b>)</h6>
            <div class="shrink-0">
                ...
                <tr>
                    ...
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Action">
                        <div class="flex gap-3">
                            <a href="{{ route('lahan.edit', $l->id_lahan) }}" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 edit-item-btn bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"><i data-lucide="pencil" class="size-4"></i></a>
                            <form id="deleteForm_{{ $l->id_lahan }}" action="{{ route('lahan.delete', $l -> id_lahan)}}" method="POST">
                                @csrf
                                <button type="submit" class="toggle-status flex items-center justify-center text-white transition-all duration-200 ease-linear rounded-md size-8 hover:text-white 
                                                    bg-red-500 delete-btn" data-id="{{ $l->id_lahan }}">
...
<div id="Tambah" modal-center="" class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-beSTEen p-4 border-b dark:border-zink-500">
            <h5 class="text-16" id="addEmployeeLabel">Tambah Lahan</h5>
        ...
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            ...
            <form class="create-form" id="create-form" action="/tambah-lahan" method="POST">
                @csrf
                <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                    <div class="xl:col-span-12">
                        <label for="lahan" class="inline-block mb-2 text-base font-medium">Lahan</label>
                        <input type="text" required id="lahan" name="lahan" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" placeholder="Nama Lahan">
                    </div>
                </div>
                <div class="xl:col-span-12">
