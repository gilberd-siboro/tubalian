...
<div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
    <ul class="flex flex-wrap items-center gap-2 mb-3 font-normal text-15">
        <li class="relative before:content-['\ea54'] before:font-remix before:ltr:-right-1 before:rtl:-left-1 before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:rtl:rotate-180 before:text-slate-500 dark:before:text-zink-200">
            <a href="/desa" class="text-slate-500 dark:text-zink-200">Desa</a>
        </li>
...
<div class="card mx-auto lg:w-2/3">
    <div class="card-body">
        <form action="{{ route('desa.update', $desa -> subdis_id)}}" method="POST">
            @csrf
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4">
                <div class="mb-3 md:col-span-2">
                    <label for="namaDesa" class="inline-block mb-2 text-base font-medium">Nama Desa</label>
                    <input required type="text" id="namaDesa" name="namaDesa" value="{{ old('namaDesa', $desa->subdis_name) }}" class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200">
                </div>
                <div class="mb-3 md:col-span-2">
                    <label for="role" class="inline-block mb-2 text-base font-medium">Kecamatan</label>
                    <select required class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200" name="kecamatan" id="kecamatan">
                        <option value="">Pilih Kecamatan</option>
                        @foreach($kecamatan as $k)
                        <option value="{{ $k->dis_id }}" {{ $k->dis_id == $desa->dis_id ? 'selected' : '' }}>{{ $k->dis_name }} </option>
                        @endforeach
                    </select>
                </div>
            </div>
